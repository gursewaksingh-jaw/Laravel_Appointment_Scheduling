...
class LicenseBoxExternalAPI{
	...
	public function __construct(){ 
		$this->product_id = '6D1DA866';
		$this->api_url = 'https://licencefly.com/';
		$this->api_key = '165EADCC719FE-Doctro';
	...
	public function activate_license($license, $client, $create_lic = true){
		return array('status' => TRUE, 'message' => 'Your license VALID | weadown.com');
		$data_array =  array(
			"product_id"  => $this->product_id,
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["8d3d9394decf2df"])){
				$_SESSION["8d3d9394decf2df"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["8d3d9394decf2df"])){
				$get_data = $this->call_api(
					'POST',
